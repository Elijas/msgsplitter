...
def get_this_message_length(i, j, chunk_lengths):
    text_character_count = sum(chunk_lengths[i:j + 1])
    space_character_count = j - i
    return text_character_count + space_character_count


def get_groups(chunk_count, chunk_lengths, formatter):
    groups = []
    i = 0
    while i < chunk_count:
        last_j = None
        for j in range(i, chunk_count):
            message_length = get_this_message_length(i, j, chunk_lengths)
...
def split_message(message: str, formatter: FormatterBase) -> List[str]:
    ...
    return [' '.join(chunks[i:j + 1]) for i, j in get_groups(chunk_count, chunk_lengths, formatter)]
